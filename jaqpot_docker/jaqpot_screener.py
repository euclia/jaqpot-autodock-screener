...
def create_simple_docker(exhaustiv, num_modes, output="./tmp"):
...
def start_docking(task_bucket, centroid, box_dims):
    ...
    df = pd.DataFrame()
    suppl = Chem.SDMolSupplier(molecule_sdf_local)
    for mol in suppl:
        run = True
        if mol is not None:
            uniqueId = mol.GetProp('_Name')
            ligand_file = "./results/" + uniqueId + ".sdf"
...
def main(args):
    ...
    create_simple_docker(exhaustiv=exhaustiveness, num_modes=num_modes)
...
#python3 jaqpot_screener.py -centroid "[0.0, 0.0, 0.0]" -t "sample_dock" -m "zinc012023/3D_BA_AAML_BAAAML.xaa.sdf.gz" -pdb "sampledocking/AF-Q7PRQ0-F1-model_v4.pdb" -box_dims "[20, 20, 20]" -s3 "localhost:9000" -s3s "False" -usn "minioadmin" -pass "minioadmin" -exhaust "1" -poses "1"